#!/usr/bin/env python3
"""Does the venue-coverage comparison compare the right things?
...
function stride_norm_track. Before 41df6c8 its ELSE branch deleted every
uppercase letter ('Canterbury' -> 'anterbury'), so a capitalised schedule row
could never match a lowercased snapshot row and the first cut of the check
(#97) reported every venue missing on a perfect day. Unit tests cannot prove
SQL semantics (they fake the cursor), so this script executes the actual
comparison against the real schema. READ-ONLY: SELECTs only, inside a
read-only session.
...
Expected once the deployed database carries the corrected function
(identity_normalization.py, applied through the repo's normal migration
...
If Aug 5 still lists Canterbury or Doomben, the deployed function is not the
corrected one. The coverage SQL mirrors the handler's check minus the
snapshot_time >= t0 freshness bound (that bound means "since this job
started", which is only meaningful inside the 08:00 job; verification asks
about the whole day). Snapshot rows age out under the 60-day retention, so
the Aug 5/6 known answers stop being checkable around 2026-10-04; pass
later dates explicitly after that.
"""
import argparse
import os
import re
