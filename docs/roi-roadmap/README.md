...
| 12 | 🟡 | 2026-09-05 | branch `claude/model-improvement-analysis-hw2mkv`: retrain_v2 gains per-race top-1 vs tip-time favourite, tail-window early stopping/isotonic (outer fold untouched), cross-fitted combiner arms measured against the production blend, WITHOUT-price ablation; v3 candidate path (retrain-model.yml `v3-candidate` → tips-proof `ensemble_artifact` → compare_candidate_tips.py); STRIDE_LEARNED_BLEND default off. No v3 trained yet: inputs-only preflight RED (5 ZERO_AT_SERVE columns; asof profiles). The 2026-09-05 pre-registration amendment and the STRIDE_LEARNED_BLEND bar were signed by Sage on 2026-09-07. **Correction 2026-09-07:** the "5 ZERO_AT_SERVE columns" above is a 2026-09-05 vintage. Do not replace it with a newer count without reading [12-retrain-rebaseline.md](12-retrain-rebaseline.md) §"Blocked on two preflight gates" first: `feature_liveness_audit.py` is **flag-blind** (zero references to any flag), and the plumbed serve block sits behind `serve_features.py:399`, so a column plumbed behind a default-off flag scores `LIVE_BOTH` while production serves a constant — the board answers "assignment code exists", not "production serves a real value", and `ZERO_AT_SERVE = 0` can pass with constants being served. `docs/research/feature_liveness_report.json` separately still holds the older 15-era counts and should be regenerated or archived. The two gates are unpacked in [12-retrain-rebaseline.md](12-retrain-rebaseline.md) §"Blocked on two preflight gates": the remaining columns are the winner-pattern features, which are **not** reachable by `STRIDE_SERVE_LIVE_FEATURES` and need plumbing or pruning as their own scheduled work; `asof-td-profiles` is a second, independent gate whose state lives on the training box |
| 13 | ⬜ | | |
| 14 | ⬜ | | |
