...
| H4 | Fitness multiplier is dead (reads a top-level key only ever published nested, 0–1 scale) | **True** | Only producer: `mc_api` → `fitnessData.fitnessReadinessScore`. Consumer reads top level, default 50 ⇒ ×1.00. Already in `SYSTEM_MAP.md §7b.1`. The jockey multiplier is inert the same way (§7b.2), which the audit did not list. Review note: mc_api already folds the jockey-momentum feature into `combined_adjustment` (22% weight) upstream, so `STRIDE_CTX_MULT_JOCKEY` applies it a second time; its A/B measures that, not an introduction. | `STRIDE_CTX_MULT_FITNESS` reads the nested 0–1 value; `STRIDE_CTX_MULT_JOCKEY` reads a `jockeyMomentumAdjustment` that mc_api now publishes (additive); `STRIDE_CTX_MULT_DIAG` prints realised min/mean/max per race. |
