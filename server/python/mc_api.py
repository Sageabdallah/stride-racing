...
def _display_kelly(win_prob, odds, fair_odds=None):
    """Fraction of bankroll for display; flag-on uses the ledger's shadow plan.

    Preserve the flag-off helper contract for legacy callers/regression pins.
    Both API emit sites keep their legacy flag-off payload unchanged.
    """
    if _flag_enabled(MC_FIX_DEAD_FIELDS):
        from portfolio_risk import shadow_kelly_display
        return shadow_kelly_display(win_prob, odds, fair_odds)['kellyStake'] / 100.0
    try:
...
def _shadow_market_fair_odds(odds, field_prices):
    """De-vig from the full active book, never mc_api's model fairOdds."""
    from market_prob import overround, true_market_prob_pct
    prices = [float(p) for p in field_prices if p and np.isfinite(float(p)) and float(p) > 1]
    if len(prices) < 2 or not odds or odds <= 1:
        return None  # display records the raw-implied fallback
    probability = true_market_prob_pct(odds, overround(prices))
    return round(100.0 / probability, 2) if probability > 0 else None


def _refresh_shadow_kelly(results, field_prices):
    """Refresh after API normalisation so the number uses served probability."""
    if not _flag_enabled(MC_FIX_DEAD_FIELDS):
        return
    from portfolio_risk import shadow_kelly_display
    for row in results:
        fair = _shadow_market_fair_odds(row.get('marketOdds'), field_prices)
        row.update(shadow_kelly_display(row.get('winPercentage', 0) / 100.0,
                                        row.get('marketOdds'), fair))
    ...
    try:
        ...
        for i, (base, mc) in enumerate(zip(analysis, mc_results)):
            ...
            if _flag_enabled(MC_FIX_DEAD_FIELDS):
                ...
                _kelly_out = round(
                    _display_kelly(win_prob / 100.0, real_market_odds) * 100, 4)
            else:
        ...
        if results:
            ...
            if _flag_enabled(MC_FIX_DEAD_FIELDS):
                _refresh_shadow_kelly(results, [parse_real_odds(r) for r in runners if is_active_runner(r)])
    ...
    try:
        ...
        if _flag_enabled(MC_FIX_DEAD_FIELDS):
            for data in races_data:
                key = (str(data.get('track', data.get('course', ''))).strip().lower(),
                       int(data.get('raceNumber', data.get('race_number', 0)) or 0))
                _shadow_books[key] = [parse_real_odds(r) for r in data.get('runners', [])
                                      if is_active_runner(r)]
        for track, tips in all_tips.items():
            ...
            for tip in tips:
                if _flag_enabled(MC_FIX_DEAD_FIELDS):
                    ...
                    # key where staking carries 'status'). Kelly display is
                    # now the same shadow estimate as the single-race endpoint;
                    # engine staking.kelly_pct remains a standalone CLI detail.
                    _tip_mc = tip.get('mc', {})
                    ...
                    from portfolio_risk import shadow_kelly_display
                    _book = _shadow_books.get((str(track).strip().lower(), int(tip.get('race_number', 0) or 0)), [])
                    _fair = _shadow_market_fair_odds(tip.get('market_odds'), _book)
                    _tip_shadow = shadow_kelly_display(_tip_mc.get('win_prob_sim', 0),
                                                       tip.get('market_odds'), _fair)
                    _tip_kelly = _tip_shadow['kellyStake']
                ...
                if _flag_enabled(MC_FIX_DEAD_FIELDS):
                    formatted_tip.update(_tip_shadow)
