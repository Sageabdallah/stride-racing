...
from market_prob import renormalise_probs
...
def load_audit_rows(input_csv: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    from fit_calibrator import load_source_frame, normalise_source_frame

    frame = normalise_source_frame(
        load_source_frame("mc_win_percentage_oof", input_csv=input_csv))
    rows = []
